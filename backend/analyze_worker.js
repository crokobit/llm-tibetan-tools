...
### 1. 結構層：單一區塊原則
* **不拆分：** **不要**將文本拆分成數個獨立區塊（如偈頌）。無論文本多長，請將**整篇文本**視為**同一個**單一區塊來處理。
* 此單一區塊的結構如下：
* ***步驟 1: 獨佔一行，使用 \`>>>\` 標記，作為開始。*
* ***步驟 2: 緊接著下方，顯示「完整的原文」。*
* ***步驟 3: 原文結束後，獨佔一行，使用 \`>>>>\` 標記。*
* ***步驟 4: 在 \`>>>>\` 下方，進行整篇的分析。*
* ***步驟 5: 整篇分析完成後，最後一行，獨佔一行，使用\`>>>>>\`標記。*
...
exports.handler = async (event) => {
    ...
    // Event comes from a direct Lambda invocation, so body is the event itself
    ...
    try {
        let responseText = '';
        console.log(`Starting analysis for Job ${jobId} using ${LLM_PROVIDER}`);
